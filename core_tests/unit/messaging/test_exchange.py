...
python -m unittest -v unit/messaging/test_exchange.py
...
class TestExchange(unittest.TestCase):
    __filename1 = Path(__file__).parent / 'data' / 'rabbitmq_exchange1.json'
    with open(__filename1) as fp:
        __jdict1 = json.load(fp)

    __filename2 = Path(__file__).parent / 'data' / 'rabbitmq_exchange2.json'
